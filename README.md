...
python scripts/training.py -c config/GO/BP/saprot.yaml

python scripts/training.py -c config/GO/MF/saprot.yaml

python scripts/training.py -c config/HumanPPI/saprot.yaml

python scripts/training.py -c config/EC/saprot.yaml

python scripts/training.py -c config/MetalIonBinding/saprot.yaml

python scripts/training.py -c config/DeepLoc/cls10/saprot.yaml
...
# General 
tmux list-sessions | grep '^sa-prot-1' | cut -d: -f1 | xargs -r -I{} tmux kill-session -t {}

srun --gres=gpu:1 -c 8 --nodelist=entropy2 --pty /bin/bash
...
export PYTHONPATH=$PYTHONPATH:/home/kalifadan/SaProt